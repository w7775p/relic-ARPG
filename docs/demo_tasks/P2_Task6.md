...
前置任务：[P2_Task 5](P2_Task5.md)。执行时以包含前置功能和 [Resource 重构](../resource_save_task.md) 的最新 `main` 为代码起点；存档重构及词条导出修复已在 `3cdc612` 合入。跨窗口接续先核对 [任务总表](README.md) 与实际代码。`313ac7f` 仅作 M2 历史对照。
...
② 同一宝箱多次交互只生成一次；实际成品中的箱内装备具有合法词条，真实 E 拾取后正确入包。开箱前、开箱后和拾取后分别撤离，回 Hub 保存恢复，持有物品与实例唯一；新探险无旧地面物品。
