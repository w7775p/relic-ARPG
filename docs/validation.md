...
分支 `feat/resource-save`，对应本轮修复提交；引擎 `4.7.2.stable.official.ed1daf0bf`。用户游玩装配为横扫。新增 `pickup_input.tscn` 经正式 Hub 装配、右键施法及 E 事件复现：横扫直接和流血击杀后的近处装备可正常拾取，远处无候选时缺少反馈，满包提示会被下一帧 HUD 覆盖。修复前 22 项中五个反馈断言失败；修复后包含异常物品反馈的 23 项全部通过。

回归同时检查按住横扫右键时 E 拾取、稳定物品身份、重复 E、背包暂停与焦点关闭、满包保留地面装备、腾格后重试及撤离到 Hub 后实际文件读回。M2 装备入包测试改为真实 E 派发；新场景已接入 `tools/verify.py`。Linux 统一入口通过导入、主入口、11 个常规场景和 7 个故障进程，无非预期错误。Windows 全量验证、导出和独立启动将在代码提交后执行。

人工待验收：Hub 装配横扫→出发→击杀后靠近装备，确认出现“E 拾取：物品名”再按 E；检查成功名称、满包持续提示和 2.5 米范围说明。若近身、有容量且出现候选仍无法拾取，保留 HUD 与 F3 日志。当前无窗口测试无法确认用户原始故障的具体条件，也不能代替目标机画面和按键手感验收。

# Resource 重构首次交付验证（2026-09-12）
