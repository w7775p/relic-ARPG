# P2_Task 1：冲锋重击与被动扩展

仓库：[w7775p/relic-ARPG](https://github.com/w7775p/relic-ARPG)。工程入口：`game/project.godot`。技术栈：Godot 4.7.2 标准版、GDScript、3D 俯视即时动作、Windows 键鼠。

阶段：D2。状态：待验收。任务 ID：`P2_Task1`。建议分支：`feat/p2-task1-charge-build`。

前置任务：[P1_Task 6](P1_Task6.md)。2026-09-21 用户已确认 D1 阶段验收通过，本卡当前已解锁。执行时仍以包含 P1_Task6 和 [Resource 重构](../resource_save_task.md) 的最新 `main` 为代码起点；PR #13 已于 2026-09-21 合入 main `6781dd8`，本卡由该最新 main 开始实现。存档重构及词条导出修复已在 `3cdc612` 合入。跨窗口接续先核对 [任务总表](README.md) 与实际代码。`313ac7f` 仅作 M2 历史对照。

## 开始前先阅读以下文档

| 文档（仓库根目录相对路径） | 阅读目的 |
| --- | --- |
| [AGENTS.md](../../AGENTS.md) | 项目约束与「完成任务前必须检查」 |
| [README.md](../../README.md) | 当前入口、操作和已实现功能 |
| [docs/engineering.md](../../docs/engineering.md) | 当前模块、数据与存档规则 |
| [docs/save_system.md](../../docs/save_system.md) | 五模块、Hub 保存、版本升级与整体回退边界 |
| [docs/tasks.md](../../docs/tasks.md) | 任务状态与前置合入情况 |
| [known_trap.md](../../known_trap.md) | 实际问题，避免重复踩坑 |
| [docs/plan.md](../../docs/plan.md) | 第 10、11 节阶段额度与验收目标 |
| [docs/demo_tasks/README.md](../../docs/demo_tasks/README.md) | 基线、执行顺序、内容归属与交接约定 |
| [docs/validation.md](../../docs/validation.md) | 现有验证入口与历史结果 |

有前置任务时同时阅读对应任务卡的验收与实际交接记录；上表文档均应存在。运行代码的最新状态优先用于判断已实现内容。

## 已经实现的功能

历史 M2 已完成战斗、随机装备、背包仓库和整备循环。当前存档基线为独立 Hub 与五模块 Resource 检查点；仅 Hub 保存，旧 JSON 和旧探险恢复已退休。历史阶段结果不替代本卡验证。

D1 验收后已有普攻、旋风、横扫、战吼四个主动技能和六个被动；碰撞、闪避和单体续航已有实现。

上述后续阶段功能须在开工时核实。本卡的“前置完成后”描述属于依赖条件，不能直接当作当前已完成事实。

## 本次要做的任务

① 新增冲锋，放入辅助槽：按瞄准方向快速位移，撞墙或达到距离时停止，沿途目标按攻击编号去重；路径受物理碰撞约束。

② 新增重击，放入基础攻击槽：可读的前摇后造成近距范围重击并击退，消耗能量且有恢复期；死亡、暂停和合法取消遵循明确规则。

③ 把冲锋与重击接成第三种节奏：冲入怪群→重击→恢复/走位；定义冲锋对下一次重击的有限增益与失效时间，参数配置化。

④ 新增四个被动，累计十个，覆盖冲锋、重击与生存/资源取舍；同时选择上限保持三个。为两个新技能补基础动作、危险范围、命中反馈和说明。

## 修改范围与 Godot 实现

| 主要路径 | 实现落点 |
| --- | --- |
| `game/systems/skills/、game/content/skills/、game/content/passives/` | 扩展 D1 装配规则，保持六个技能的稳定 ID 与标签。 |
| `game/actors/player/player_controller.gd、game/systems/combat/、game/shared/vfx/` | CharacterBody3D 处理冲锋运动；攻击结算仍进入 CombatSystem。 |
| `game/systems/skills/、game/ui/inventory/` | 运行期动作与冷却提示；持久装配接入 character 模块。 |

涉及路径以当前仓库检查为准；标注新增的目录由本任务按实际功能建立。共用存档入口为 `game/app/services/save_manager.gd`、`game/systems/persistence/` 与 `game/world/hub/hub.gd`，仅在本卡确有影响时修改；相关回归放 `game/tests/` 并接入 `tools/verify.py`。节点和 API 先查项目现有用法，新增用法核对同版官方文档。新增类、函数、回调与功能写简明中文注释。

## 存档与兼容

冲锋阶段、命中集合、重击动作和组合增益属于当前探险，暂停时保持、离场时清理，不保存到检查点。主动/被动选择归 character 模块，若结构变化提供 Resource 升级，旧角色保持原选择。

## 验收标准

① 面对墙角、薄墙、敌人和狭窄入口验证冲锋停止，禁止瞬移穿墙；一个冲锋对同一目标只结算约定次数。

② 重击伤害发生在规定时点；暂停不跳过前摇，死亡/撤离清理动作；Hub 保存重启后新探险没有残留攻击。

③ 使用测试装备独立打通普通怪群与单精英，记录能量循环；正式专属装备由 P2_Task7 接入。

④ 累计六个主动技能、十个被动均可选择和解释，已有两套构筑通过受影响回归。

功能验证通过 `python tools/verify.py --godot <引擎路径>` 执行，包含源码场景、存储故障及隔离 PCK 的拾取/存档回归；新增用例需要实际运行到完成标记。交付 Windows 包时按 [成品验收约定](README.md#成品验收约定) 验证实际 exe。显示、声音和手感按本次可见试玩记录验收。

## 要求

1. 先检查当前项目状态：分支、工作区、最新 main、前置合入、实际代码与验证入口；保护现有未提交修改。先完成可以核实的工作，具体依赖缺失时说明缺失文件或功能。
2. 不要实现与本任务无关的系统。只参考本仓库与所需官方资料，沿用本项目约定，无需使用 `global-work-rules`。
3. 优先最小可用实现，以本卡验收为完成边界；从当前主入口可以观察到功能，必要存档与基础反馈同批完成。具体数值可调整并记录原因。
4. 完成后按 AGENTS.md「完成任务前必须检查」逐条执行，更新本卡状态、任务总表和工作日志；实际遇到新坑才更新 `known_trap.md`。每完成一个独立且验证通过的子任务立即 commit，再同步任务分支和 PR（合并请求）；合并按用户当前授权执行。
5. 最后按下表汇报，并附分支/提交、测试命令与实际结果、PR 和可用构建链接。

| 汇报项 | 必须包含 |
| --- | --- |
| 改了哪些文件 | 主要文件及用途；文档、资源、存档版本变化。 |
| 当前能做什么 | 从哪个入口操作，得到什么结果，哪些验收已经实际通过。 |
| 还不能做什么 | 未完成项、失败项、未验证项及对应影响。 |
| 下一步建议做什么 | 已解锁的下一张任务卡、待合入依赖或具体阻塞。 |

## 执行记录

2026-09-21 从最新 main `6781dd8`（PR #13 已合入）创建 `feat/p2-task1-charge-build`。首个子任务接通 F 冲锋、实际胶囊碰撞、沿途一次命中与限时增益。Godot 4.7.2 实际运行 `tests/p2_charge.tscn`：22 项通过，覆盖薄墙、墙角、狭口、敌人和暂停。当前继续重击与四被动；全量、Windows 和可见验收尚未运行。

第二个子任务：重击动作拆为 HeavyAttack；0.35 秒前摇、0.45 秒恢复、28 能量与 3.2 米范围，按攻击快照结算。空格、死亡、离场取消，暂停保持；组合增益在合法起手消费。`p2_heavy.tscn` 22 项通过。四被动及整体验证接续。

第三个子任务：新增破阵（冲锋 +50%）、重锤（重击 +35%）、重甲（护甲 +20 / 移速 -1 米每秒）、换息（闲置回能 +8/秒 / 生命上限 -40）。保持最多三项，UI 说明读取真实配置。`p2_loadout.tscn` 29 项通过，覆盖正式 Hub 页面、真实 F/左键/空格、Resource 文件往返、暂停背包与新探险无残留。character 结构与版本保持，旧选择保留。

第四个子任务：第三构筑使用 `debug/charge_test.tres` 独立通过六只普通怪群与一名精英，保留敌人 AI；5 项断言通过。固定 60 Hz 逻辑记录：怪群 0.50 秒、最低能量 66、结束 100；精英 14.62 秒、最低 0.10、结束 13.10、低于重击费用 763 帧，完成恢复后续战。此为测试遭遇数据，未代表整图或目标机手感。成品共用 smoke 新增正式 Hub 装配→真实 F/左键→精英掉落→E 拾取→Hub 保存读回，源码 47 项通过。长技能说明移到自动换行区域，装配回归增至 31 项，Hub 页面回归 12 项通过。下一步为最终全量、隔离 PCK、Windows CI 与交付记录；可见画面、声音与手感待人工验收。


## 当前交付与接续

分支 `feat/p2-task1-charge-build`；[PR #14](https://github.com/w7775p/relic-ARPG/pull/14)，尚未合并。远端独立提交为冲锋 `9dedbe9`、重击 `091237b`、被动与保存 `9cd0d43`、遭遇与成品覆盖 `4a389e5`。本地验证代码与远端 `4a389e5` 文件树一致。

2026-09-21 用官方 `4.7.2.stable.official.ed1daf0bf` 执行 `python tools/verify.py --godot <引擎路径>`，退出码 0：22 个源码场景、7 类存储故障、隔离 PCK 全部通过。新增冲锋 22、重击 22、装配 31、遭遇 5 项；共用拾取/存档在源码与包内均 47 项通过。既有雷霆旋风与流血横扫回归通过。[Windows run 35584816275](https://github.com/w7775p/relic-ARPG/actions/runs/35584816275) 全量、导出与实际 exe 检查通过；[可用构建](https://github.com/w7775p/relic-ARPG/actions/runs/35584816275/artifacts/10631808873)，日志与哈希见 [validation.md](../validation.md)。

当前入口：Hub“技能与被动”选择基础槽重击、辅助槽冲锋，并最多选三项被动；出发后 F 冲入目标，再用左键重击。配置信息与暂停/取消规则见 [工程说明](../engineering.md#p2_task1-冲锋重击)。持久结构未变化，所有 Resource 模块版本保持；专属装备仍由 P2_Task7 负责。

未验证范围：本环境没有可用图形服务，尚未实际确认窗口画面、范围提示可读性、声音听感和键鼠手感；1280×720 的容器边界断言仅证明布局几何。人工按下列路径验收：在 1280×720 与 1920×1080 读取完整技能/被动说明；冲锋撞墙角、薄墙、狭口和怪物；命中后 3 秒内重击与过期重击对照；前摇时 I/Esc 暂停、空格取消；怪群/精英战后 T 回城保存、重启再出发确认无残留动作。没有已知自动验证失败。

下一步：完成上述人工验收并合入 PR #14，再从最新 main 执行 P2_Task2“墓园敌人与精英组合”。本卡状态保持待验收，不将后续功能列为已完成。
