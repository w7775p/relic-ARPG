...
func _refresh_loadout() -> void:
	...
	attributes.text = "已选 %d/3｜%s\n装备＋被动合计：伤害 %.1f｜暴击 %.0f%%｜旋风半径 %.1f 米\n生命上限 %.0f｜护甲 %.0f｜停止施放回能 %.1f/秒\n%s\nQ · %s" % [state.passives.size(), "据点可免费调整" if hub else "探险中只能查看", build.damage, build.critical_chance * 100, build.whirlwind_radius_m, session.inventory.defense("max_health"), session.inventory.defense("armor"), build.idle_energy_regen, SkillDefinition.damage_rule_text(), LoadoutState.POTION.describe(build)]
