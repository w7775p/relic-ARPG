...
前置任务：[P2_Task 9](P2_Task9.md)。执行时以包含前置功能和 [Resource 重构](../resource_save_task.md) 的最新 `main` 为代码起点；存档重构及词条导出修复已在 `3cdc612` 合入。跨窗口接续先核对 [任务总表](README.md) 与实际代码。`313ac7f` 仅作 M2 历史对照。
...
③ 在已有多槽和结构化结果基础上统一保存/继续提示：Hub 手动槽、自动历史、坏档或缺失内容回退、重试及读取不自动覆盖。区分实际保存成功与无变化跳过，失败保留会话，正常退出等待成功。拾取失败使用拾取结果与物品诊断，保存反馈使用实际保存结果。
...
① 新存档从启动到首次换装可以按界面独立完成；跳过和重看均正常，操作完成后提示推进且不遮挡关键战斗。拾取引导以物品成功入包为完成条件，无候选、满包或物品校验拒绝时保留该步骤并显示对应原因。
