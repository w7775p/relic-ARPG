...
阶段：D3。状态：待执行。任务 ID：`P3_Task6`。建议分支：`feat/p3-task6-release`。

前置任务：[P3_Task 1](P3_Task1.md)；[P3_Task 2](P3_Task2.md)；[P3_Task 3](P3_Task3.md)；[P3_Task 4](P3_Task4.md)；[P3_Task 5](P3_Task5.md)。执行时以包含前置功能和 [Resource 重构](../resource_save_task.md) 的最新 `main` 为代码起点；存档重构及词条导出修复已在 `3cdc612` 合入。跨窗口接续先核对 [任务总表](README.md) 与实际代码。`313ac7f` 仅作 M2 历史对照。
...
Windows CI 已运行源码与隔离 PCK 回归，导出内嵌资源的 exe 后通过 Boot 的 --smoke-loot 完成包内生成、真实拾取与 Hub 存档验证。共用用例位于 app/validation，tests/debug 仍被导出排除；SkillRunner 仍直接预加载基础/雷霆测试预设。

上述后续阶段功能须在开工时核实。本卡的“前置完成后”描述属于依赖条件，不能直接当作当前已完成事实。

## 本次要做的任务

① 完成 Demo 发布模式：主菜单进入最终据点和墓园，清除正式入口的调试热键、测试预设入口和测试资源依赖；保留开发分支中的回归与压力工具。

② 把技能的生产默认数据与调试预设依赖分开，再验证导出排除规则；检查导出的资源引用闭包，避免删掉预设后游戏无法启动。

③ 更新 Windows 工作流、产物命名、版本信息和成品验证。沿用 --smoke-loot 与 tools/verify_package.py，共用用例随正式 Demo 场景更新，覆盖配置加载、击杀拾取、Hub 保存读回和下一件掉落。测试会话使用隔离用户目录，仅由专用参数启动；玩家正常启动保持正式新档/继续流程。

④ 生成可下载 Windows 包、操作说明、版本变更、已知问题、资产/字体/引擎署名与许可证文件；清单来自已登记资源，核对实际随包内容。

⑤ 在干净 Windows 环境实际运行最终导出包，完成新档首次流程、旧档继续、三套构筑至少各一次首领、设置保存和退出重开；记录构建提交与验证证据。

## 修改范围与 Godot 实现

| 主要路径 | 实现落点 |
| --- | --- |
| `game/export_presets.cfg、game/app/boot/、game/systems/skills/、game/content/` | 消除生产对调试数据的依赖，同时保留开发测试。 |
| `.github/workflows/windows.yml、tools/verify.py、tools/verify_package.py、game/app/validation/、builds/（忽略产物）` | 同版引擎与模板导出，检查退出码、错误日志、成品玩法完成标记与产物；保留包内用例所需资源。 |
...
① 干净检出后完整回归与 Windows 导出通过；对最终交付 exe 执行成品验证，确认词条配置、真实拾取、保存读回及下一次掉落正确。日志和下载产物对应同一提交，包内具有所需资源。

② 发行包能完成三套构筑与首领流程，调试快捷键/成型预设无法从正式界面访问；开发回归入口仍可运行。

③ 旧档恢复、改键、中文字体、声音、保存退出重开和目标机性能都有对应记录，未执行项明确保留。

④ 交付版本号、提交、下载地址、操作说明、已知问题与署名；Demo 是否可交付按已完成证据判断。
