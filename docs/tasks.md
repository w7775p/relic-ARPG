...
| M2-02 | M2 | 掉落、拾取与背包 | M2-01 | 战利品、背包、穿脱 | 换装立即影响战斗 | 拾取反馈修复与横扫真实按键 23 项双平台通过；Windows 新包见 validation.md，原始现场待复核 |
...
| [Resource 存档](resource_save_task.md) | 五模块、多角色多槽、Hub 检查点、整体回退、旧 JSON/旧探险移除 | 实现完成；PR #8 待评审与人工可见验收 | 11 个常规场景＋7 个故障进程；包含拾取跟进的 Windows 回归/导出/独立启动通过，详见 validation.md |
