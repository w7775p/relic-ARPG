...
前置任务：[P3_Task 1](P3_Task1.md)。执行时以包含前置功能和 [Resource 重构](../resource_save_task.md) 的最新 `main` 为代码起点；存档重构及词条导出修复已在 `3cdc612` 合入。跨窗口接续先核对 [任务总表](README.md) 与实际代码。`313ac7f` 仅作 M2 历史对照。
...
① 将移动、主要技能、拾取、候选切换、背包、保存换键，注入真实键盘/鼠标事件验证，重启保持；重复键冲突处理与恢复默认正常。同步共用成品用例，覆盖按住主要技能时按新拾取键入包，以及关闭背包后的输入焦点。
