...
前置任务：[P2_Task 6](P2_Task6.md)。执行时以包含前置功能和 [Resource 重构](../resource_save_task.md) 的最新 `main` 为代码起点；存档重构及词条导出修复已在 `3cdc612` 合入。跨窗口接续先核对 [任务总表](README.md) 与实际代码。`313ac7f` 仅作 M2 历史对照。
...
② 在源码与实际 Windows 成品中按固定来源和种子生成装备，覆盖普通怪、精英、宝箱、首领及目标独特；逐件核对部位/等级/品质规则并通过存档校验。对已知小权重表核对分布，容差按样本量说明。
